...
public struct Album
{
    ...
    internal Album(NativeAlbum albumNativeStruct) : this()
    {
        MapFromNative(ref albumNativeStruct);
    }

    private void MapFromNative(ref NativeAlbum nativeStruct)
    {
    ...
    internal void SendAlbum(nint mptDeviceStructPointer, bool createNew)
    {
        GCHandle tracksHandle = GCHandle.Alloc(Tracks, GCHandleType.Pinned);
        try
        {
            nint tracksPtr = tracksHandle.AddrOfPinnedObject();
            var albumNativeStruct = new NativeAlbum()
            {
                album_id = AlbumId,
                parent_id = ParentId,
            ...
            if (createNew)
                Native.LibMtp.CreateNewAlbum(mptDeviceStructPointer, ref albumNativeStruct);
            else
                Native.LibMtp.UpdateAlbum(mptDeviceStructPointer, ref albumNativeStruct);
            MapFromNative(ref albumNativeStruct);
        }
        finally
        {
