...
namespace Nmtp.Native
{
    ...
    public partial class LibMtp
    {
        ...
        private static extern int LIBMTP_Send_Track_From_Handler(IntPtr device, MtpDataGetFunction getFunc, IntPtr priv,
            ref Track metadata, ProgressFunction? callback, IntPtr data);

        public static int SendTrackFromHandler(IntPtr device, MtpDataGetFunction getFunction, ref Track trackStruct, 
        ...
        [DllImport(LibMtpName)]
        private static extern int LIBMTP_Create_New_Album(IntPtr device, ref NativeAlbum metadata);

        public static int CreateNewAlbum(IntPtr device, ref NativeAlbum albumNativeStruct)
        {
        ...
        /// <returns>0 on success, any other value means failure.</returns>
        [DllImport(LibMtpName)]
        private static extern int LIBMTP_Update_Album(IntPtr device, ref NativeAlbum metadata);
        
        public static int UpdateAlbum(IntPtr device, ref NativeAlbum albumNativeStruct)
        {
        ...
        [DllImport(LibMtpName)]
        private static extern int LIBMTP_Send_Representative_Sample(IntPtr device, uint id, ref NativeFileSampleData sampledata);

        public static int SendRepresentativeSample(IntPtr device, uint id, ref NativeFileSampleData sampledata)
        {
